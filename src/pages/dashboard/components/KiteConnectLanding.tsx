...
interface KiteConnectLandingProps {
  onConnect: () => void;
}

const KiteConnectLanding = ({ onConnect }: KiteConnectLandingProps) => {
  return (
    <motion.div
    ...
    >
      {/* Hero */}
      <section className="relative z-10 mx-auto flex w-full max-w-4xl flex-col items-center pb-10 pt-10 text-center">
        <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-emerald-400/20 bg-emerald-400/10 px-4 py-1.5 text-xs font-semibold uppercase tracking-[0.24em] text-emerald-300">
          Step 1 of 1 — Connection Required
        </div>

        <h1 className="max-w-3xl text-4xl font-semibold leading-[1.1] tracking-[-0.04em] text-white sm:text-5xl lg:text-6xl">
      ...
      {/* Main connect card */}
      <section className="relative z-10 w-full max-w-2xl pb-10">
        <div className="relative overflow-hidden rounded-2xl border border-white/10 bg-white/5 p-8 shadow-[0_30px_80px_rgba(0,0,0,0.5)] sm:p-10 backdrop-blur-md">
          {/* Glow accent */}
          ...
          {/* Feature bullets */}
          <ul className="relative mt-6 space-y-3 font-light">
            {[
          ...
          <div className="relative mt-8 flex flex-col gap-3 sm:flex-row">
            <button
              ...
              onClick={onConnect}
              className="inline-flex flex-1 items-center justify-center gap-2 rounded-xl bg-emerald-400 px-6 py-3.5 text-sm font-semibold text-slate-950 shadow-[0_0_24px_rgba(52,211,153,0.3)] transition hover:bg-emerald-300 hover:shadow-[0_0_32px_rgba(52,211,153,0.45)] active:scale-[0.98] border-none cursor-pointer"
              type="button"
            >
              Connect to Zerodha
...
export default KiteConnectLanding;
